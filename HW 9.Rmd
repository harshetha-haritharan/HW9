...
groceries <- groceries %>%
  mutate(Income10K = Income / 10000,
         z.Income10K = scale(Income10K))

# Fit models
model_income <- lm(Price ~ Product + Income10K, data = groceries)
model_z_income <- lm(Price ~ Product + z.Income10K, data = groceries)

# Extract needed coefficients
coefs_income <- summary(model_income)$coefficients["Income10K", ]
coefs_z_income <- summary(model_z_income)$coefficients["z.Income10K", ]

# Combine into table
f_table <- data.frame(
  Model = c("Income10K", "z.Income10K"),
  Estimate = c(round(coefs_income["Estimate"], 3),
               round(coefs_z_income["Estimate"], 3)),
  Std_Error = c(round(coefs_income["Std. Error"], 3),
                round(coefs_z_income["Std. Error"], 3)),
  P_value = c(round(coefs_income["Pr(>|t|)"], 3),
              round(coefs_z_income["Pr(>|t|)"], 3))
)

kable(f_table, caption = "Effect of Income on Price")
```
Consumers in poorer ZIP codes seem to pay slightly more for the same product because the coefficient for Income10K is –0.014 which means that for every $10,000 increase in income, the average product price decreases by about 1.4 cents. However, the p-value is 0.144 meaning that it is not not statistically significant at the 5% level, therefore we can not be confident the relationship exists in the larger population. A one-standard deviation increase in the income of a ZIP code seems to be associated with a 0.064 standard-deviation decrease in the price that consumers in that ZIP code expect to pay for the same product.
...
This statement is false because there is no evidence present in the figures or models suggesting an interaction effect between minority percentage and the housing age in predicting FAIR policies. Model B studies the relationship between minority and age, with a R-squared value of 0.06 and a p-value of 0.125, showing a weak value and also that it is not statistically significant. Figure B1 shows no clear trend between housing age and minority percentage. The points are spread out, and the flat line suggests a weak relationship. There is no model that has an interaction between these variables and also no figure that has the stratified trends by age. To understand this interaction, we need a regression model that has an interaction term of minority * age. Another thing that is needed is a plot that shows the relationship between minority and FAIR policies differs across different age levels, such as younger vs. older housing ZIPs. 
...
This statement is true and can be supported by Figure C1. This figure shows two trend lines and it can be seen that the high fire risk group has a steeper positive slope compared to the low fire risk group which has a flatter slope. In model_C it can be seen that the minority:fire_riskLow interaction value is –0.001 meaning that  the minority effect is slightly weaker in low-fire-risk ZIPs, but it is not significant as the p-value = 0.839. In the high-fire-risk, the base minority slope value is 0.01, and is significant as the p-value is 0.015  meaning we have good evidence of this positive association in high-risk areas. This supports that the minority percentage predicts FAIR policies in high-fire-risk ZIPs, however it is not in the low-risk-fire ZIP codes. 
...
This statement is false because although income does reduce the strength of the association between minority percentage and FAIR policy uptake, it does not explain it entirely. In model D1, there is no control for any other variables, and the coefficient for the minority is 0.014 and statistically significant. After controlling for income in model D2, the coefficient for minority = decreases to 0.010 and is still statistically significant as p = 0.002. Even though the income is also statistically significant as p = 0.041, the inclusion does not eliminate the effect of the minority. This shows that income accounts for a part of the association, however, the minority percentage is still the independent effect of FAIR policy rates. Therefore, a more accurate statement would be that controlling for income reduces the association between minority percentage and FAIR policy uptake, but does not eliminate it and the minority percentage continues to show an independent effect. This is supported by model D2, where the minority coefficient stays significant even after controlling for income. Its value of 0.010 shows that the positive connection with FAIR policy uptake still exists. The decrease from 0.014 in model D1 to 0.010 in model D2 suggests that income explains part of the relationship, but not all of it.
...
This statement is true because even after being controlled for multiple factors, the minority percentage is still significantly associated with FAIR policies at the ZIP code level. In model E, the fully adjusted model, the coefficient for the minority is 0.008 with a p-value of 0.006, showing that there is a statistically significant positive relationship. The model includes controls for income, fire risk, and housing age, but only the minority and fire risk values are significant values. The R-squared value is 0.662 showing that the model explains a large portion of the variance in FAIR policy rates. This gives strong evidence that the association between minority percentage and FAIR policy use is not explained by other qualities including income, fire risk, or housing age. 